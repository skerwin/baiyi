...
class ChannelVideoController: BaseViewController ,Requestable {
    ...
    func loadData(){
        ...
        if menupagetype == MenuPageType.Channel {
            let ChnnelParams = HomeAPI.AllListPathAndParams(page: 1, pageSize: 10, obj_type: 2, is_recommended: 0, group_id: 0, keyword: "")
            postRequest(pathAndParams: ChnnelParams,showHUD: true)
            
            let video_slide = intForKey(key: "video_slide") ?? 0
            let SlideParams =  HomeAPI.SlideListPathPathAndParams(id: video_slide)
            
            postRequest(pathAndParams: SlideParams,showHUD: false)
            
        }else if menupagetype == MenuPageType.ColumForDetail{
            let ChnnelParams = HomeAPI.AllListPathAndParams(page: page, pageSize: 10, obj_type: 2, is_recommended: 0, group_id: cid, keyword: "")
            postRequest(pathAndParams: ChnnelParams,showHUD: false)
        }else if menupagetype == MenuPageType.HomeMore{
            
            let HomeMoreParams = HomeAPI.AllListPathAndParams(page: 1, pageSize: 10, obj_type: 2, is_recommended: 0, group_id: 0, keyword: "")
            ...
            let requestParams = HomeAPI.mineCollectPathAndParams(obj_type: 2, pageSize: pagenum, page: page)
            postRequest(pathAndParams: requestParams,showHUD: true)
        }else if menupagetype == MenuPageType.MineGood{
            isMineViewAppear = true
            let requestParams = HomeAPI.mineGoodListPathAndParams(obj_type: 2, pageSize: pagenum, page: page)
            postRequest(pathAndParams: requestParams,showHUD: true)
        }
        else{
    ...
    override func onResponse(requestPath: String, responseResult: JSON, methodType: HttpMethodType){
        ...
        if requestPath == HomeAPI.mineCollectPath || requestPath == HomeAPI.mineGoodListPath {
            ...
            for model in MineList {
                rankvideoModelList.append(model.video!)
            }
            
         }else{
            
            list = getArrayFromJsonByArrayName(arrayName: "list", content:  responseResult)
            rankvideoModelList.append(contentsOf: list)
            if list.count < 10 {
                self.tableView.mj_footer?.endRefreshingWithNoMoreData()
            }
        }
    ...
    func initTableView(){
        ...
        tableView.registerNibWithTableViewCellName(name: VideoListCell.nameOfClass)
...
extension ChannelVideoController:SDCycleScrollViewDelegate{
...
extension ChannelVideoController:UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if menupagetype == MenuPageType.MineCare || menupagetype == MenuPageType.MineGood {
            tableView.tableViewDisplayWithMsg(message: "暂无数据", rowCount: rankvideoModelList.count ,isdisplay: true)
        }else{
            tableView.tableViewDisplayWithMsg(message: "", rowCount: rankvideoModelList.count ,isdisplay: true)
        }
        if rankvideoModelList.count == 0{
             return 0
        }
        
        return rankvideoModelList.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        cell.model = rankvideoModelList[indexPath.row]
        //cell.delegeta = self
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        controller.fid = rankvideoModelList[indexPath.row].id
        self.navigationController?.pushViewController(controller, animated: true)
    }
}
